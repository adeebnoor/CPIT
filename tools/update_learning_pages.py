...
s=s.replace('practise, then defend a decision.', 'practise, then defend a bounded professional decision.')
...
expected_times=dict(re.findall(r"'([0-9]+)':\{[^\n]+?time:'([^']+)'",(ROOT/'fbr-submission.html').read_text()))
for i,(ch,d) in enumerate(CHAPTERS.items(),1):
    pattern=rf'(<article class="lec"[^>]*id="ch{ch}".*?)(</article>)'
    def patch(m):
        card=re.sub(r'<div class="stage-note">.*?</div>','',m[1],flags=re.S)
        card=re.sub(r'<span>FBR · [^<]+</span>',f'<span>FBR · {e(expected_times[str(ch)])}</span>',card,count=1)
