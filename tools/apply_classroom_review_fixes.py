...
Fix legacy-entry routing and distinguish title/map/table layouts in DOM checks.
...
def patch(name,before,after):
    ...
    assert s.count(before)==1,name+' does not match the reviewed correction'
...
patch('tools/learning-path-tests/check-visual.cjs',
      "assert(x.d.querySelectorAll('.points li').length>0)",
      "assert(x.d.querySelectorAll('.points li').length>0||x.d.querySelector('.table-main,.two-approaches'),'Concept screen must contain its points or technical comparison table')")
...
review['review_corrections']=['Resolve legacy START entry through explicit chapter aliases.','Check source comparison tables directly rather than requiring duplicate bullets.','Exercise existing hub anchors in the real-origin test.']
