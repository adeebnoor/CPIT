...
def _source_first_items(u: LectureUnit, limit: int = 5) -> list[tuple[str, str]]:
...
def _ppt_frame(slide, bp: Blueprint, u: LectureUnit, *, show_decision: bool = False):
    ...
    if show_decision:
        _ppt_text(slide, .65, 6.60, .68, .22, "YOU TRY", 8.5, RED, True)
        _ppt_text(slide, 1.38, 6.56, 9.35, .34, _display_question(u), 10.8, INK, True)
...
def _r_frame(c, bp: Blueprint, u: LectureUnit, show_decision=False):
    ...
    if show_decision:
        ...
        _r_wrap(c, _display_question(u), 95, 51, 650, 8.2, R_INK, True, 1)
...
def render_cimt_presenter_preview_v43(bp: Blueprint, release_state: str = "BLOCKED") -> str:
    ...
    for i, (u, plan) in enumerate(zip(bp.units, plans)):
        ...
        if u.number == 1:
            ...
            decision = f'<div class="decision"><b>YOU TRY</b><span>{_h(_display_question(u))}</span></div>' if show_decision else ''
            content = f'''<div class="corner"></div><header class="head"><div><h2>{_h(_display_title(bp, u))}</h2><p class="question">{_h(_display_question(u))}</p></div><div class="mark"><i></i><b>CIMT</b><small>ISCARB</small></div></header><div class="visual">{visual}</div>{decision}<footer class="foot"><span>{_h(_subject(bp))} · source-grounded presenter</span><em>{u.number:02d}/20</em></footer>'''
