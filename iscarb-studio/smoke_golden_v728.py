...
probe = NS(lecture_title="Dependable systems",engineering_thesis="Dependability requires evidence and bounded assumptions",source_topic_families=["Redundancy and diversity"])
case = scaffold._micro_case(probe).lower(); assert "redund" in case and "power" in case, case
...
bp = NS(units=units, release_notes=[]); v728._operationalize_rule19(bp); u19=bp.units[18]
assert u19.title == "Peer-review quick card" and len(u19.core_content)==2
assert "independently inspectable" in u19.core_content[0].lower(); assert "what variable" in u19.core_content[1].lower()
spec={"title":"Formal methods — mechanism, benefits, adoption limits","source_anchor":"[P1] SLIDES 39–45","content":["Formal specification can expose inconsistencies before implementation."]}
out=v728._add_decision_box_to_specs([spec])[0]
assert out["content"][0].startswith("DECISION EVIDENCE BOX") and "artifact or test" in out["content"][0].lower()
print("PASS: Golden v6.6 + time-boxing + scaffold + peer-review + Source Expansion decision boxes")
